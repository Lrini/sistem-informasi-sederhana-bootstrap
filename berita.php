    ...
    if($movie){
		$deskripsi = $_POST['deskrip_berit'];
		$id_unit = $_POST['id_unit'];
		$rawid = $_POST['id_admin'];
		$array = explode(" - ", $rawid);
		$id_adm = $array[0];
		echo "id = " . $id_adm;
        $sql = mysql_query("INSERT INTO berita (foto_ber,deskrip_berit,id_unit,id_admin)values ('$fileName','$deskripsi','$id_unit','$id_adm')");
        ...
        if($sql){
          ...
          <?php
			...
			?>
          <script type="text/javascript">
            alert('Gagal');window.location='berita.php';
          </script>
          <?php
		}
      }
			  ...
			  <div class="form-group">
				 ...
				 <div class="form-group">
                  <label for="exampleInputPassword1">Isi berita</label>
					<textarea name="deskrip_berit" id="news"></textarea>
						<script type="text/javascript">
				...
				session_start();
				$name = $_SESSION['nama_admin'];
				$resid = mysql_query("select admin.id_admin from admin where nama_admin = '" . $name . "'");
